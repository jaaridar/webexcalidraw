...
function CanvasList({ onSelect }: { onSelect: (id: string | null) => void }) {
  ...
  useEffect(() => {
    // Load all canvas IDs from localStorage (or KV if you want server-side)
  ...
  const createNew = () => {
    ...
    const updated = [...new Set([newId, ...canvasIds])].slice(0, 100); // Keep last 100
  ...
  const deleteCanvas = (id: string) => {
    const updated = canvasIds.filter(cid => cid !== id);
    setCanvasIds(updated);
    localStorage.setItem('canvas-list', JSON.stringify(updated));
  };

  if (loading) return <div style={{ padding: '2rem' }}>Loading canvases...</div>;
...
export default function App() {
  ...
  const loadCanvas = async (id: string) => {
    try {
      const res = await fetch(`${API_BASE}/load?id=${id}`);
      if (res.ok) {
  ...
  useEffect(() => {
    saveCanvasRef.current = debounce(async (data: { elements: any[]; appState: any }) => {
      if (!canvasId) return;
      try {
  ...
  if (isLoading || !initialData || !canvasId) {
